   ...
   // Active paggination & chage data
	...
	const AcceptRequest = async (id) => {
		...
		try {
			...
			const response = await axiosInstance.put("/api/bxg/"+id, requestBody).catch((error) => {
				console.log(error);
			});
	...
	const FetchData = async () => 
	{
		...
		try
		{
			...
			setpendingrequests(data.filter(item => item.type == "Pending"));
                            ...
                            <div className="card-header border-0 pb-2 flex-wrap">
                                ...
                                <>
                                    <Nav className="order nav nav-tabs">
                                        <Nav.Link as="button"  eventKey="All"  type="button">Pending Requests</Nav.Link>
                                        <Nav.Link as="button"  eventKey="Order" type="button">Accepted Requests</Nav.Link>
                                        <Nav.Link as="button"  eventKey="Trade" type="button">Rejected Requests</Nav.Link>
                                    </Nav>
                                </>
                            </div>
                                        ...
                                        <div className="table-responsive dataTabletrade ">
                                            <div id="future_wrapper" className="dataTables_wrapper no-footer">
                                                ...
                                                <div className="d-sm-flex text-center justify-content-between align-items-center mt-3 mb-3">
                                                   {pendingrequests.length>0?( <div className="dataTables_info">
                                                        Showing {activePag.current * sort + 1} to{" "}
                                                        {pendingrequests.length > (activePag.current + 1) * sort
                                                            ? (activePag.current + 1) * sort
                                                            : pendingrequests.length}{" "}
                                                        of {pendingrequests.length} entries
                                                    </div>):(<div className="dataTables_info">loading...</div>)}
                                                    <div
